...
test('check weather data display after switching forecast options', async ({ page }) => {
    let locationInput = page.locator('#location-input');
    await locationInput.fill('Amsterdam');
    await locationInput.press('Enter');
    await page.waitForSelector('.container', { state: 'visible' });

    const dailyForecastButton = await page.waitForSelector('button:has-text("Daily Forecast")');
    ...
    const weeklyForecastButton = await page.waitForSelector('button:has-text("7-Day Forecast")');
    ...
    const fourteenDaysForecastButton = await page.waitForSelector('button:has-text("14-Day Forecast")');
